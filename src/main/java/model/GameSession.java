...
/**
 * GameSession represents a single session of the Minesweeper game.
 * It follows the Singleton pattern to ensure only one active session exists.
 * This class manages game state, player actions, scores, and game flow.
 */
public class GameSession {
    ...
    /** Timestamp for when the session began. */
     ...
     * 
     * @param leftPlayerName  Name of the left player
     * @param rightPlayerName Name of the right player
     * @param gameDifficulty  Difficulty setting for the game
     */
    ...
    /**
     * Retrieves the singleton instance of the GameSession.
     * Initializes with default values if it doesn't exist.
     * 
     * @return The single GameSession instance.
     */
    ...
    /**
     * Retrieves the test singleton instance of the GameSession.
     * Initializes with default values if it doesn't exist.
     * 
     * @return The test GameSession instance.
     */
     ...
     * Checks if enough questions exist for the selected difficulty,
     * resets stats, and initializes the boards.
     * 
     * @throws IllegalStateException if there are insufficient questions.
     */
    ...
    /** Initialize game stats (points and health). */
    ...
    /**
     * Sets the health pool. Health cannot go below 0.
     * If health exceeds max, extra health is converted to points.
     * 
     * @param i New health value
     */
    private void setHealthPool(int i) {
        this.healthPool = i;
        if (this.getHealthPool() < 0)
            this.healthPool = 0;
    ...
    // ==================================================================================
    // GAMEPLAY ACTIONS (INPUT HANDLING)
    // ==================================================================================

    /**
     * Handles the logic when a user right-clicks a tile (flagging/unflagging).
     * Responsible for switching turns, calculating points, and updating board
     * state.
     * 
    ...
    public void RightClickedTile(Tile tile) {
        /*
         * encompasses all the logic that happens when a user tries to flag/unflag a
         * tile
         * responsible for switching turns, gaining points and ordering board to
         * flag/unflag
         */
        message = "";
    ...
    /**
     * Handles the logic when a user left-clicks a tile (revealing).
     * Manages turn switching, mine detonation, point calculation, and special tile
     * activation.
     * 
     * @param tile The tile that was left-clicked.
     * @return true if the action was valid and processed, false otherwise.
     */
    public boolean LeftClickedTile(Tile tile) {
        /*
         * encompasses all the logic that happens when a user tries to reveal a tile
         * responsible for switching turns, gaining points and ordering board to reveal
         */
        message = "";
    ...
    /**
     * Activates the special effect of a tile (Surprise or Question).
     * Deducts the activation cost and triggers the specific listener event.
     * 
     * @param specialTile The special tile being activated.
     * @param parentBoard The board containing the tile.
     * @return true if activation was successful (enough points), false otherwise.
     */
    private boolean activateSpecialTile(SpecialTile specialTile, Board parentBoard) {
        System.out.println(
                "Activation cost: " + getGameDifficulty().getActivationCost() + "\tPoints: " + this.getPoints());
        ...
        if (this.getPoints() < getGameDifficulty().getActivationCost()) {
            ...
            if (specialTile instanceof QuestionTile questionTile) {
                message = "Question tile activated and it costed you " + getGameDifficulty().getActivationCost()
                        + " points";
                notifyListenersAfterAction(message, true, 0, -getGameDifficulty().getActivationCost());
                System.out.println("Question tile activated");
                displayQuestionListener.displayQuestion(parentBoard);
            }
            specialTile.setUsed();
            System.out.println("Points: " + " " + this.getPoints() + "    Health: " + this.getHealthPool() + "\n");
        }
    ...
    /**
     * Create a string specifically for when the user gains extra points from extra
     * hp.
     */
    private String extraHpString(int currentHealth, int healthReward) {
        ...
        if (potentialHealth > MAX_HEALTH_POOL) {
            ...
            if (actualHpGain > 0) {
                return " (Gained " + actualHpGain + " HP + " + pointsFromHealth + " pts from " + overflowAmount
                        + " extra HP)";
            } else {
    ...
    public void updateAfterQuestionResult(QuestionDifficulty difficulty, boolean correctAnswer, Board parentBoard) {
        ...
        SoundManager.getInstance()
                .playOnce(correctAnswer ? SoundManager.SoundId.POINTS_WIN : SoundManager.SoundId.POINTS_LOSE);
    ...
    private void updateAfterQuestionResultEasy(QuestionDifficulty difficulty, boolean correctAnswer,
            boolean randomResult, Board parentBoard) {
    ...
    private void updateAfterQuestionResultMedium(QuestionDifficulty difficulty, boolean correctAnswer,
    ...
    private void updateAfterQuestionResultHard(QuestionDifficulty difficulty, boolean correctAnswer,
            boolean randomResult) {
        // Original values before the changes
        int startPoints = getPoints();
        int startHealth = getHealthPool();
     ...
     * 
     * @return true if health <= 0 or all mines revealed on either board.
     */
    private boolean isGameOver() {
        return getHealthPool() <= 0 || getLeftBoard().allMinesRevealed() || getRightBoard().allMinesRevealed();
    }

    /** Forces the game to end immediately (e.g. from debug). */
     ...
     * Reveals boards, calculates final score, notifies listeners, and saves the
     * game.
     */
    private void initiateGameOver() {
        if (isGameOverProcessing)
            return; // Prevent multiple saves/overlays
        ...
        if (getHealthPool() > 0) {
            ...
            setHealthPool(0);
            notifyListenersAfterAction("Game over! You won!", true, -healthBefore,
                    healthBefore * getGameDifficulty().getActivationCost());
        } else
    ...
    private void saveGame(boolean winOrLose) throws IOException {
        GameData gameData = new GameData(this.timeStamp, this.leftPlayerName, this.rightPlayerName, this.gameDifficulty,
                this.points, winOrLose);
    ...
    /**
     * Notifies all action listeners of a game event.
     */
    ...
    // clear listeners to avoid memory leaks when playing more than 1 game per
    // running instance
